...
# ==========================================
# 1. SETUP & MANUAL CLICKING (From Listing 1)
# ==========================================
N = 6  # Let's use 6 points for a more stable manual homography
n = 0
p1 = np.empty((N, 2))
p2 = np.empty((N, 2))
...
def draw_circle(event, x, y, flags, param):
    global n
    p = param[0]
    # Record point on left mouse click, up to N points
...
if im1 is None or im2 is None:
    print("Error: Could not load c1.jpg or c2.jpg. Check your file paths!")
    exit()
...
print(f"--- STEP 1: Click {N} distinct points on Image 1 ---")

while True:
    cv.imshow("Image 1", im1copy)
    if n == N:
        break
    if cv.waitKey(20) & 0xFF == 27: # ESC key to exit early
        break
...
print(f"--- STEP 2: Click the exact SAME {N} corresponding points on Image 2 ---")
...
print("\nPoints collected! Processing Homography and SIFT algorithms...\n")

# ==========================================
# 2. PARTS (a) & (b): MANUAL HOMOGRAPHY
# ==========================================
...
# ==========================================
# Generate a clean matplotlib grid to easily screenshot for your report
plt.figure(figsize=(16, 9))
plt.suptitle("IT5437 Assignment 2: Circuit Board Alignment", fontsize=16)

# Manual Results Row
plt.subplot(2, 3, 1)
plt.imshow(cv.cvtColor(warped_im1_manual, cv.COLOR_BGR2RGB))
plt.title("Manual Warped Image")
plt.axis('off')

plt.subplot(2, 3, 2)
plt.imshow(diff_thresh_manual, cmap='gray')
plt.title("Manual Difference (Thresholded)")
plt.axis('off')

# SIFT Results Row
plt.subplot(2, 3, 4)
plt.imshow(cv.cvtColor(warped_im1_auto, cv.COLOR_BGR2RGB))
plt.title("Automated SIFT Warped Image")
plt.axis('off')

plt.subplot(2, 3, 5)
plt.imshow(diff_thresh_auto, cmap='gray')
plt.title("Automated SIFT Difference")
plt.axis('off')
